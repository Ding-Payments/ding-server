 ...
 *
 * PrismaService is mocked so no live DB is required.
 * The JWT guard is bypassed by injecting a fake user via overrideProvider.
 */
...
/**
 * Fake guard: injects a test user into every request so protected routes
 * behave as if authenticated.  Public routes are still allowed through.
 */
class FakeAuthGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    const req = context.switchToHttp().getRequest<Record<string, unknown>>();
    req['user'] = { supabaseUserId: 'test-supabase-user-id' };
    return true;
  }
}
...
describe('PaymentRequests persist + retrieve (e2e)', () => {
  ...
  describe('GET /v1/payment-requests/:id', () => {
    ...
    it('returns 404 when the payment request does not exist', async () => {
      ...
      const res = await request(app.getHttpServer()).get(
        `${PERSIST_ROUTE}/00000000-0000-0000-0000-000000000000`,
      );
